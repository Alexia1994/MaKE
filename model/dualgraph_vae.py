...
class EncoderGGNN(nn.Module):
    def __init__(self, vocab_size, node_dim, hidden_dim, n_hop=5):
        ...
        self.out1 = nn.Linear(self.node_dim * 2, hidden_dim)
...
class Attention(nn.Module):
    ...
    def forward(self, decoder_state, encoder_outputs, input_node_mask):
        ...
        if input_node_mask is not None:
            ...
            pass
        # log_score = torch.log_softmax(u_i, dim=-1)
        log_score = torch.softmax(u_i, dim=-1)
...
class DecoderVAE(nn.Module):
    ...
    def forward(self, prev_y_batch, prev_h_batch, equ_encoder_outputs, sns_encoder_outputs, z_sample,input_equ_node_mask, input_sns_node_mask):
        ...
        plan_attn_weight = torch.softmax(self.plan_w(prev_h_batch), dim=-1)
        ...
        # dec_output: [batch_size, 1, hidden_size*2+z_dim]
        # dec_hidden: [batch_size, hidden_size*2+z_dim]
        dec_output, dec_hidden = self.my_rnn_gate(rnn_input.unsqueeze(1), prev_h_batch.unsqueeze(0))
        
        # dec_output: [batch_size, 1, output_size]
...
class WordRNN(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim,dropout=0.1, pretrained=None):
        ...
        # 将不可训练的tensor转换为可训练的parameter, 同时绑定进模型
    ...
    def forward(self, one_doc):
        ...
        '''
        # one_doc: [batch_size, max_len] 
        seq_len = len(one_doc[0])
        # need fix: batch size?
        batch_size = len(one_doc)
        # 和0相等的部分输出1
        ...
        # [batch_size * hidden_dim * hidden_dim] X [batch_size, hidden_size, seq_len] ——> [batch_size, hidden_size, seq_len]
        A = torch.tanh(torch.bmm(self.Ws1.repeat(1*batch_size, 1, 1), final_T))
        # [batch_size * 1 * hidden_dim] X [batch_size, hidden_size, seq_len] ——> [batch_size, 1, seq_len]
        # Ws1和Ws2参数共享
        A = torch.bmm(self.Ws2.repeat(1*batch_size, 1, 1), A)
        # [batch_size, seq_len]
        A = A.squeeze(1).masked_fill(tmp_mask, -1e12)
        # [batch_size, 1, seq_len]
        A = F.softmax(A, dim=1).view(1*batch_size, -1, seq_len)
        # [batch_size, 1, seq_len] X [batch_size, seq_len, hidden_size] ——> [batch_size, 1, hidden_size] —> [batch_size, hidden_size]
...
class Graph2seq(nn.Module):
    ...
    def sample_z(self, mu, logvar):
        ...
        # torch.randn_like: 返回与输入相同大小的张量，由区间[0, 1]均匀分布的随机数填充
    ...
    def decode_dual(self, dec_input_var, cond_embedding, equ_encoder_outputs, sns_encoder_outputs, \
        ...
        # z_sample 在train时采用的是后验分布，inference时采用的是先验分布
        batch_size, seq_len = dec_input_var.shape
        dec_hidden = torch.cat([cond_embedding, z_sample], dim=1)
        # 使用BOS来作为开头
        dec_input = torch.tensor([Constants.BOS] * batch_size, dtype=torch.long).to(device)
        predicted_logits, graph_attention = [], [] # use this to record the attention score
        # d_dec = d_initial
        for di in range(seq_len):
            ...
            graph_attention.append(plan_attn)
        ...
        planning_probs = torch.stack(graph_attention, 1)
